...
tiff('figs/occ_seasons.tif', pointsize = 8, compression = "lzw", res=300, width = 1100, height = 900)
ggplot(occ.all,aes(x=x,y=y,group=interaction(col, season),colour=factor(col),shape=factor(season))) +
...
tiff('figs/tvol_seasons.tif', pointsize = 8, compression = "lzw", res=300, width = 1100, height = 900)
ggplot(tvol.all,aes(x=x/1000,y=y,group=interaction(col, season),colour=factor(col),shape=factor(season))) +
  geom_line(size=0.3,aes(linetype=season)) +
  ylab("Likelihood of Collision") +
  xlab("Traffic Volume (1000 vehicles/day)") +
  labs(color = "Species") +
...
tiff('figs/tspd_seasons.tif', pointsize = 8, compression = "lzw", res=300, width = 1100, height = 900)
ggplot(tspd.all,aes(x=x,y=y,group=interaction(col, season),colour=factor(col),shape=factor(season))) +
  geom_line(size=0.3,aes(linetype=season)) +
  ylab("Likelihood of Collision") +
  xlab("Traffic Speed (km/hour)") +
  labs(color = "Species") +
